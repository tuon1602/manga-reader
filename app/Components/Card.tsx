          ...
          <Link href={`/manga/${route}`}>
            <Card className="max-h-content shadow max-sm:border-0">
              ...
              <CardHeader className="max-sm:py-1 max-sm:px-0 px-2">
                <CardTitle className="truncate text-lg max-sm:text-xs">
                  {name}
                </CardTitle>
              ...
              <CardContent className="max-sm:px-0 pb-2 px-2 text-sm">
                ...
                {lastestChapter &&
                  lastestChapter.map((item, index) => (
                    <h3
                      key={index}
                      className="mt-[5px] truncate max-sm:text-xs"
                    >
                      Tập{" "}
                ...
                {author && (
                  <h2>
                    Tác giá:{" "}
                    <span className="text-primary font-bold truncate max-sm:text-sm max-sm:hidden">
                      {author}
                    </span>
                  </h2>
                )}
              ...
              <CardFooter className="max-sm:hidden flex flex-wrap text-sm gap-2 px-2 max-lg:px-1">
                {category.slice(0, 2).map((item, index) => (
                  <p
                    key={index}
                    className="px-2 border border-primary rounded-3xl max-lg:text-xs max-lg:gap-1 "
                  >
                    {item.name}
                  </p>
                ))}
